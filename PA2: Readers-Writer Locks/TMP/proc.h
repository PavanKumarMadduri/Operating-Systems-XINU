...
struct	pentry	{
	...
	int	pwaitret;
	int 	lockid;			/* In whose wait queue the process is blocked. */
	int 	ptype;			/* State(READ-WRITE) of process in Wait Queue. */
	int	proclock[50];		/* READ-WRITE type for each lock per process */
	int	pclock[50];		/* Time at which the process is pushed into the lock queue */
