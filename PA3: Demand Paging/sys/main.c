...
void proc_test1(int *ret) {
	...
	*(addr0) = 'D';
	...
	kprintf("xunmao b11111111111111efore\n");
	for (i=0; i<16; i++){
	    *addr1 = 'A'+i;
	    addr1 += NBPG;
	}
	...
	for (i=0; i<16; i++){
		...
		addr1 += NBPG;
		kprintf("insdoe for loop\n");
	}
	kprintf("xunmao before\n");
	xmunmap(0x40000000>>12);
	 kprintf("xunmao after\n");
	release_bs(MYBS1);
	 kprintf("release after\n");
	return;
}
...
void proc_test2(int i,int j,int* ret,int s) {
	...
	r = xmmap(MYVPNO1, i, j);
	if (j<=50 && r == SYSERR){
		kprintf("Return-1-r-%d,j-%d\n",r,j);
		kprintf("loop1\n");
		*ret = TFAILED;
	}
	if (j> 50 && r != SYSERR){
		kprintf("Return-2-r-%d,j-%d\n",r,j);
		kprintf("loop2\n");
		*ret = TFAILED;
	}
	kprintf("Current - PID-%d\n", currpid);
